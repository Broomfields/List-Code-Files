...
void search_dir(const wchar_t* dir_name, const wchar_t* csv_file_name)
{
	_WDIR* dir;
	struct _wdirent* ent;
	...
	if (dir != NULL)
	{
		...
		while ((ent = _wreaddir(dir)) != NULL)
		{
			...
			if (ent->d_type == DT_DIR)
			{
				...
				search_dir(path, csv_file_name);
			}
			// Check if the entry is a file with the ".h" extension
			else if (wcsstr(ent->d_name, L".h") != NULL)
			{
...
int wmain(int argc, wchar_t* argv[])
{
	...
	wchar_t csv_file_name[1024];
	swprintf(csv_file_name, sizeof(csv_file_name) / sizeof(wchar_t), L"%ls.csv", argv[1]);

	// Search the specified directory for files with the ".h" extension
	search_dir(argv[1], csv_file_name);
